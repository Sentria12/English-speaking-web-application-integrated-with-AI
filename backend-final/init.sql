...
-- 8. Bảng Speaking Evaluations (Dữ liệu cho biểu đồ)
-- Tạo bảng này để khớp với file SpeakingEvaluation.java bạn đã gửi
CREATE TABLE `speaking_evaluations` (
  `evaluation_id` BIGINT NOT NULL AUTO_INCREMENT,
  `learner_id` INT NOT NULL,
  `mentor_id` INT DEFAULT NULL,
  `booking_id` INT DEFAULT NULL,
  `pronunciation_score` DOUBLE DEFAULT 0.0,
  `grammar_score` DOUBLE DEFAULT 0.0,
  `fluency_score` DOUBLE DEFAULT 0.0,
...
-- 9. Dữ liệu mẫu để biểu đồ có đường nét (Test Dashboard)
-- Giả sử learner_id = 1 (Nguyễn Văn Học)
INSERT INTO `speaking_evaluations` (learner_id, pronunciation_score, created_at) VALUES 
(1, 6.0, DATE_SUB(NOW(), INTERVAL 6 DAY)),
(1, 6.5, DATE_SUB(NOW(), INTERVAL 5 DAY)),
(1, 7.2, DATE_SUB(NOW(), INTERVAL 4 DAY)),
(1, 7.0, DATE_SUB(NOW(), INTERVAL 3 DAY)),
(1, 7.8, DATE_SUB(NOW(), INTERVAL 2 DAY)),
(1, 8.2, DATE_SUB(NOW(), INTERVAL 1 DAY)),
(1, 8.5, NOW());

USE aesp;
INSERT INTO `speaking_evaluations` (learner_id, pronunciation_score, grammar_score, fluency_score, feedback_details)
VALUES (1, 8.5, 8.0, 7.5, 'Tốt lắm!');